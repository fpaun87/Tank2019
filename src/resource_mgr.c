...
static bool font2Tex(int fontTexId, int size, SDL_Color color)
{
    ...
    TTF_Font *pFont = TTF_OpenFont(FONT_PATH"/Monoid-Bold.ttf", size);
    if(!pFont)
    {
    ...
    SDL_FreeSurface(pTextSurface);
    rsmgr.texTable[fontTexId].pTex = pTextTex;
    return true;
}
...
static bool loadTexture(int texId, const char * path)
{
    ...
    rsmgr.texTable[texId].pTex = pTex;
...
{
    return rsmgr.texTable[texId].pTex;
}
...
bool rsmgrInit(void)
{
    ...
    if(!loadTexture(TEX_ID_BRICK, SPRITE_PATH"/brick2.png"))
        return false;
    ...
    if(!font2Tex(TEX_ID_PLAY_FONT,24, (SDL_Color){0, 0, 0, 255}))
            return false;

    if(!font2Tex(TEX_ID_GAMEOVER_FONT, 100, (SDL_Color){135, 0, 0, 255}))
            return false;
...
void rsmgrClose(void)
{
    ...
    {
        if(rsmgr.texTable[i].pTex != NULL)
            SDL_DestroyTexture(rsmgr.texTable[i].pTex);
    }
